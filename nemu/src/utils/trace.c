...
int func_num = 0;
int parse_elf32_functions_with_size(ElfFunction **out_funcs, char *elf_data) {
        ...
        if (shdrs[i].sh_type == SHT_SYMTAB || shdrs[i].sh_type == SHT_DYNSYM) {
            ...
            for (size_t j = 0; j < sym_count; j++) {
                ...
                if (ELF64_ST_TYPE(syms[j].st_info) == STT_FUNC && syms[j].st_name != 0) {
                    ...
                    uint64_t start = syms[j].st_value;
                    uint64_t size = syms[j].st_size;
                    ...
                    if (count >= capacity) {
                        capacity *= 2;
                        ElfFunction *tmp = realloc(funcs, capacity * sizeof(ElfFunction));
                        if (!tmp) goto cleanup;
                        funcs = tmp;
                    }
...
int init_elf_file(const char *elf_file)  {
    ...
    func_num = parse_elf32_functions_with_size(&func_list, elf_data);
