...
//  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1< src2){s->dnpc = s->pc + imm;});//not test
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = Mr(src1 + (sword_t)imm, 1));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = Mr(src1 + (sword_t)imm, 2));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + (sword_t)imm, 4));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));

  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + (sword_t)imm, 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + (sword_t)imm, 2, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + (sword_t)imm, 4, src2));
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, if((src1 < src2) | (src2 == 0)) {R(rd) = 1;}\
                                                                else {R(rd) = 0;} );
