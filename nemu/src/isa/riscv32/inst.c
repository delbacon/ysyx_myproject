...
//指令名称作为注释，指令类型用于后续译码
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, {R(rd) = csr_read(BITS(inst, 31, 20)), csr_write(BITS(inst, 31, 20), src1);} );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = csr_read(BITS(inst, 31, 20)); /*csr_write(imm, src1 | csr_read(imm))*/; );
