...
//BEQ and BNE take the branch if registers rs1 and rs2 are equal or unequal respectively
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1==src2){s->dnpc = imm;});//also beqz
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1!=src2){s->dnpc = imm;});//also benz?
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if(src1 < imm || src1 == 0){R(rd) = 1;} else {R(rd) = 0;});//also seqz

  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , I, R(rd) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , I, R(rd) = (word_t)(src1 - src2));
