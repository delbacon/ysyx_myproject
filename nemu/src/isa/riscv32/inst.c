...
//BEQ and BNE take the branch if registers rs1 and rs2 are equal or unequal respectively
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1!=src2){s->dnpc = s->pc + imm;});//also benz?
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if((word_t)src1 < (word_t)BITS(imm ,11 ,0) || src1 == 0){R(rd) = 1;} else {R(rd) = 0;});//also seqz
