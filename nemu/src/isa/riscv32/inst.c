...
//  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if(src1< src2){s->dnpc = s->pc + imm;});//not test
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(src1>=src2){s->dnpc = s->pc + imm;});//not test
...
//  INSTPAT("0000000 ????? ????? 010 ????? 00100 11", slli   , I, R(rd) = src1 << BITS(inst, 24, 20));
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", srli   , I, R(rd) = src1 >> BITS(inst, 24, 20));
