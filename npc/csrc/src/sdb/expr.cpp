...
int find_main_op(int p, int q) 
{
  ...
  for (int i = p; i <= q; i++) {
    ...
    //否则判断运算优先级
      ...
      if (tmp_type > op_pre || (tmp_type == op_pre && TOKEN_TYPES(tokens[i].type, Operators))) {//bug!!!
        op_pre = tmp_type;
        ret = i;
      }
    }
...
static word_t eval_operation(int p, bool *legal) { 
  ...
  case TK_HEX:
    ...
    break;
  case TK_REG:
    return reg_str2val(tokens[p].str, legal);
    break;
