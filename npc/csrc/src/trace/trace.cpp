...
int func_num = 0;
int parse_elf64_functions_with_size(ElfFunction **out_funcs, char *elf_data) {
    //获取 elf 头部结构体
    ...
    //释放动态分配的内存（realloc会尝试调用之前分配的内存块，如果size=0且对应位置已分配，则返回NULL）
...
extern "C" int parse_elf32_functions_with_size(ElfFunction **out_funcs, char *elf_data) {
    //获取 elf 头部结构体
    //Elf32_Ehdr 描述了文件整体的布局信息
...
int parse_elf_functions_with_size(ElfFunction **out_funcs, char *elf_data) {
    ...
    if(e_ident[EI_CLASS] == ELFCLASS32) {
        printf("[parse_elf_functions_with_size()]:ELF32 file detected\n");
        return parse_elf32_functions_with_size(out_funcs, elf_data);
    }else if(e_ident[EI_CLASS] == ELFCLASS64){
        printf("[parse_elf_functions_with_size()]:ELF64 file detected\n");
...
static char *ftrace_get_func_name(word_t pc){
    if (func_num < 0) {
        return NULL;
    } else {
        int func_start = 0;
